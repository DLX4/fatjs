 ...
 * 06.
 */
public class SematicValidator extends FatjsBaseListener {
    ...
    @Override
    public void exitVariableDeclarator(FatjsParser.VariableDeclaratorContext ctx) {
        super.exitVariableDeclarator(ctx);
    }

    @Override
    public void exitVariableDeclaratorId(FatjsParser.VariableDeclaratorIdContext ctx) {
    ...
    private boolean hasReturnStatement(ParseTree ctx){
        ...
        for (int i = 0; i< ctx.getChildCount(); i++){
            ...
            if (child instanceof FatjsParser.StatementContext &&
                    ((FatjsParser.StatementContext)child).RETURN() != null){
                rtn = true;
                break;
            }
            else if (!(child instanceof FatjsParser.FunctionDeclarationContext ||
                child instanceof FatjsParser.ClassDeclarationContext)){
    ...
    private boolean checkBreak(RuleContext ctx){
        if (ctx.parent instanceof FatjsParser.StatementContext &&
                (((FatjsParser.StatementContext) ctx.parent).FOR() != null ||
                ((FatjsParser.StatementContext) ctx.parent).WHILE() != null) ||
        ctx.parent instanceof FatjsParser.SwitchBlockStatementGroupContext){
            return true;
        }
        else if (ctx.parent == null || ctx.parent instanceof FatjsParser.FunctionDeclarationContext){
            return false;
        }
        else {
